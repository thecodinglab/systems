...
}:
{
  ...
  config =
    ...
    in
    lib.mkIf config.custom.i3.enable {
      xsession.windowManager.i3 = {
        ...
        config = rec {
          ...
          startup = [
            {
              command = "${pkgs.xss-lock}/bin/xss-lock --transfer-sleep-lock -- ${pkgs.xsecurelock}/bin/xsecurelock";
              always = true;
              notification = false;
            }
          ];
      ...
      home.file.".background-image" = {
        enable = true;
        # TODO: add to ./wallpaper/default.nix
